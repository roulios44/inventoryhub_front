            ...
            <div class="container py-3">
                ...
                <ul class="list-group">
                    <li class="list-group-item d-flex justify-content-between align-items-center"
                        v-for="entity in filteredEntities" :key="entity.id" @click="toDetails(entity)">
                        <span>Order number : {{ entity.orderNumber }}</span>
                        <span>Type : {{ entity.orderType }}</span>
                    </li>
                </ul>
    ...
    setup() {
        ...
        const getEntries = async () => {
            try {
                ...
                const reqSupplier = await axios.get(`${apiUrl}/supplierOrders`, { headers });
                const resSupplier = reqSupplier.data._embedded.supplierOrders;
                resSupplier.forEach(element => { element.orderType = "supplier" })
                const reqClient = await axios.get(`${apiUrl}/clientOrders`, { headers });
                const resClient = reqClient.data._embedded.clientOrders;
                resClient.forEach(element => {
                    element.orderType = "client"
                });
                entities.value = [...resClient, ...resSupplier]
        ...
        const toDetails = (entity) => {
            const type = entity.orderType
            const id = entity.id
            router.push(`/order/details/${type}/${id}`);
        };
        watch(
            () => route.params.type,
            (newType) => {
